...
export default function LandlordDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!mLand) { navigate("/login-locador"); return; }
      setLandlord(mLand);
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          setForm({ ...form, image: canvas.toDataURL("image/webp", 0.8) });
        }
      };
      ...
      <main className="container py-8">
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
            <form onSubmit={handleSave} className="w-full max-w-lg rounded-xl border border-border/60 bg-card p-6 space-y-4 animate-scale-in">
